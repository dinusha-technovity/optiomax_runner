...
class NotifyMaintenanceTasksAssetSchedule extends Command
{
    protected $signature = 'notify:maintenance-tasks-asset-schedule';
    protected $description = 'Notify responsible persons maintenance tasks based on asset maintenance schedules.';
    ...
    protected function processTenant($tenant)
    {
        ...
        // Process asset_item_maintenance_tasks
        $schedules = $db->table('asset_item_maintenance_tasks')
        ...
        foreach ($schedules as $schedule) {
            ...
            if ($this->isDue($schedule->schedule, $now)) {
                ...
                $exists = $db->table('asset_item_action_queries')
                    ...
                    ->where('source', 'asset_item_maintenance_tasks_schedule_check')
                    ->exists();
                ...
                if (!$exists) {
                    $queryId = $db->table('asset_item_action_queries')->insertGetId([
                        ...
                        'source' => 'asset_item_maintenance_tasks_schedule_check',
                        'tenant_id' => $tenant->id,
                        'created_at' => now(),
                        'updated_at' => now(),
                    ]);
        ...
        // Process maintenance_tasks
        $groupSchedules = $db->table('maintenance_tasks')
            ->whereNull('deleted_at')
            ->where('isactive', true)
            ->get();
            ...
            if ($this->isDue($schedule->schedule, $now)) {
                ...
                foreach ($assetItems as $assetItem) {
                    $exists = $db->table('asset_item_action_queries')
                        ...
                        ->where('tenant_id', $tenant->id)
                        ->whereDate('created_at', $now->toDateString())
                        ->where('source', 'asset_maintenance_tasks_schedule_check')
                        ->exists();
                    ...
                    if (!$exists) {
                        $queryId = $db->table('asset_item_action_queries')->insertGetId([
                            ...
                            'reading_id' => 0,
                            'recommendation_id' => $schedule->id,
                            'source' => 'asset_maintenance_tasks_schedule_check',
                            'tenant_id' => $tenant->id,
                            'created_at' => now(),
                            'updated_at' => now(),
                        ]);

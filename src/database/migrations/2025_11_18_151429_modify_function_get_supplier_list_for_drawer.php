    ...
    public function up(): void
    {
        ...
        BEGIN
            ...
            IF p_supplier_id IS NULL THEN
                ...
                v_where_clause := 'AND s.supplier_reg_status = ''APPROVED''';
                v_message := 'Approved suppliers fetched successfully';

            ELSIF p_supplier_id = -1 THEN
                -- Fetch ALL invited suppliers
            ...
            ELSIF p_supplier_id = 0 THEN
                -- Fetch all suppliers, ignore reg_status
                v_where_clause := '';
                v_message := 'All suppliers (reg status ignored) fetched successfully';

            ELSE
                -- Fetch a specific supplier, but only approved
                v_where_clause := format(
                    'AND s.id = %s AND s.supplier_reg_status = ''APPROVED''',
                    p_supplier_id
                );

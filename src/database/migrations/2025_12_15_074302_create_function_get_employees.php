    ...
    public function up(): void
    {
        ...
        DB::unprepared(<<<'SQL'
            ...
            DECLARE
                ...
                v_offset_curr INT;
                v_offset_next INT;
                v_order_clause TEXT := 'ORDER BY e.id DESC'; -- default sorting
            ...
            BEGIN
                ...
                CASE LOWER(TRIM(p_sort_by))
                    WHEN 'newest' THEN v_order_clause := 'ORDER BY e.id DESC';
                    WHEN 'oldest' THEN v_order_clause := 'ORDER BY e.id ASC';
                    WHEN 'az' THEN v_order_clause := 'ORDER BY e.employee_name ASC NULLS LAST';
                    WHEN 'za' THEN v_order_clause := 'ORDER BY e.employee_name DESC NULLS LAST';
                    ELSE v_order_clause := 'ORDER BY e.id DESC';
                END CASE;
                ...
                FROM employees e
                LEFT JOIN designations d ON d.id = e.designation_id AND d.deleted_at IS NULL
                ...
                AND (
                    p_search IS NULL OR
                    e.employee_name ILIKE '%' || p_search || '%' OR
                    e.employee_number ILIKE '%' || p_search || '%' OR
                    e.email ILIKE '%' || p_search || '%' OR
                    e.phone_number ILIKE '%' || p_search || '%' OR
                    CONCAT('+', e.contact_no_code::TEXT, ' ', e.phone_number) ILIKE '%' || p_search || '%' OR
                ...
                inner_sql := format('
                    ...
                    FROM employees e
                    LEFT JOIN organization org ON org.id = e.department AND org.deleted_at IS NULL
                    LEFT JOIN designations d ON d.id = e.designation_id AND d.deleted_at IS NULL
                    LEFT JOIN users u ON u.id = e.user_id
                    WHERE e.tenant_id = %L
                    AND e.deleted_at IS NULL
                    AND e.isactive = TRUE
                    AND (%L IS NULL OR e.id = %L)
                    AND (
                        %L IS NULL OR
                        e.employee_name ILIKE ''%%'' || %L || ''%%'' OR
                        e.employee_number ILIKE ''%%'' || %L || ''%%'' OR
                        e.email ILIKE ''%%'' || %L || ''%%'' OR
                        e.phone_number ILIKE ''%%'' || %L || ''%%'' OR
                        CONCAT(''+'', e.contact_no_code::TEXT, '' '', e.phone_number) ILIKE ''%%'' || %L || ''%%'' OR
                ...
                ',
                    ...
                    p_search, p_search, p_search, p_search, p_search, p_search, p_search,

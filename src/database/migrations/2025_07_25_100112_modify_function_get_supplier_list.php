...
return new class extends Migration
{
    ...
    public function up(): void
    {
        DB::unprepared(<<<SQL
            ...
            AS $$
            BEGIN
                -- Check if both tenant ID and supplier ID are not provided then return all
                ...
                IF p_supplier_id IS NOT NULL AND p_supplier_id < -1 THEN
                    RETURN QUERY 
                    SELECT 
                        'FAILURE',
                ...
                IF p_supplier_id = -1 THEN
                    ...
                    SELECT
                        'SUCCESS',
                        'All invited suppliers fetched successfully',
                        s.id,
                        COALESCE(s.name, '')::TEXT,
                        COALESCE(s.description, '')::TEXT,
                        s.email::TEXT,
                        COALESCE(s.supplier_reg_status, 'INVITED')::TEXT,
                        COALESCE(s.supplier_rating, 0)::BIGINT
                    FROM suppliers s

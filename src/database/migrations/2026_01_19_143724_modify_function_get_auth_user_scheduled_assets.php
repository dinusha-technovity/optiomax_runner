                            ...
                            jsonb_build_object(
                                ...
                                'assigned_employees', (
                                    SELECT jsonb_agg(
                                        jsonb_build_object(
                                            'employee_id', u2.id,
                                            'employee_name', u2.name,
                                            'employee_number', u2.employee_number,
                                            'user_id', u2.id,
                                            'user_name', u2.name,
                                            'user_profile_image', u2.profile_image
                                        )
                                    )
                                    FROM asset_schedule_related_employees asre2
                                    LEFT JOIN users u2 ON asre2.employee_id = u2.id
                                    WHERE asre2.asset_schedule_id = eas2.id
                                    AND u2.employee_account_enabled = TRUE
                                )
                ...
                ORDER BY
                    ...
                    (
                        EXISTS (
                            ...
                            INNER JOIN users emp_sort ON asre_sort.employee_id = emp_sort.id

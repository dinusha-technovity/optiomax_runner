...
return new class extends Migration
{
    /**
     * Run the migrations.
     * Updates insert_or_update_asset_booking to set booked_by_user_id when optiomesh_customer_id is null
     */
        ...
        DB::unprepared(<<<'SQL'
            ...
            BEGIN
                FOR r IN
                    ...
                    FROM pg_proc
                    WHERE proname = 'insert_or_update_asset_booking'
                LOOP
            ...
            DECLARE
                ...
                v_slot_sequence_counter INT := 0;
                v_time_slot_sequence_counter INT;
                v_matching_occurrence_id BIGINT;
            ...
            BEGIN
                ...
                IF p_id IS NULL OR p_id = 0 THEN
                    ...
                    INSERT INTO asset_bookings (
                        ...
                        p_booking_type_id,
                        p_parent_booking_id,
                        CASE WHEN p_optiomesh_customer_id IS NULL THEN TRUE ELSE FALSE END, -- Internal booking if no customer
                        p_optiomesh_customer_id,
                        CASE WHEN p_optiomesh_customer_id IS NULL THEN p_booked_by_user_id ELSE NULL END, -- Set user_id only when customer is null
                ...
                FOR v_idx IN 0..jsonb_array_length(p_booking_items) - 1 LOOP
                    ...
                    v_slot_results := '[]'::JSONB;
                    v_time_slot_sequence_counter := 0;
    ...
    {
        // Restore previous version - reverts to setting booked_by_user_id as NULL for external bookings
        DB::unprepared(<<<'SQL'
            CREATE OR REPLACE FUNCTION insert_or_update_asset_booking(
                IN p_id BIGINT DEFAULT NULL,
                IN p_booking_number VARCHAR DEFAULT NULL,
            ...
            AS $$
            BEGIN
                -- Reverted to original behavior where booked_by_user_id is always NULL for external bookings
                RETURN QUERY SELECT 'REVERTED'::TEXT, 'Function reverted to previous version'::TEXT, NULL::BIGINT, NULL::VARCHAR, NULL::JSONB;
            END;
            $$;
        SQL);
    }

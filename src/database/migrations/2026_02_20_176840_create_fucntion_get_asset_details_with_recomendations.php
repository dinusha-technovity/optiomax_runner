<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    public function up(): void
    {
        DB::unprepared(<<<SQL
        DO $$
        DECLARE
            r RECORD;
        BEGIN 
            FOR r IN
                SELECT oid::regprocedure::text AS func_signature
                FROM pg_proc
                WHERE proname = 'get_asset_details_with_recommendations'
            LOOP
                EXECUTE format('DROP FUNCTION %s CASCADE;', r.func_signature);
            END LOOP;
        END$$;

        CREATE OR REPLACE FUNCTION get_asset_details_with_recommendations(
            p_tenant_id BIGINT,
            p_asset_item_id BIGINT DEFAULT NULL,
            p_user_id BIGINT DEFAULT NULL,
            p_action TEXT DEFAULT NULL
        )
        RETURNS TABLE (
            status TEXT,
            message TEXT,
            id BIGINT,
            asset_id BIGINT,
            asset_name VARCHAR,
            model_number VARCHAR,
            serial_number VARCHAR,
            thumbnail_image JSONB,
            qr_code JSONB,
            asset_tag VARCHAR,
            assignee_type JSON,
            assets_type_id BIGINT,
            assets_type_name VARCHAR,
            category_id BIGINT,
            category_name VARCHAR,
            sub_category_id BIGINT,
            sub_category_name VARCHAR,
            manufacturer VARCHAR,
            department_id BIGINT,
            department_data JSONB,
            maintenance_recommendations JSONB,
            recommendations JSONB
        )
        LANGUAGE plpgsql
        AS $$
        BEGIN
            -- Tenant check
            IF p_tenant_id IS NULL OR p_tenant_id <= 0 THEN
                RETURN QUERY SELECT 
                    'FAILURE','Invalid tenant ID provided',
                    NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
                    NULL,NULL,NULL,NULL,NULL,NULL,NULL::VARCHAR,NULL::BIGINT,NULL::JSONB,
                    NULL::JSONB,NULL::JSONB;
                RETURN;
            END IF;

            -- Asset item check
            IF p_asset_item_id IS NOT NULL AND p_asset_item_id < 0 THEN
                RETURN QUERY SELECT 
                    'FAILURE','Invalid asset item ID provided',
                    NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
                    NULL,NULL,NULL,NULL,NULL,NULL,NULL::VARCHAR,NULL::BIGINT,NULL::JSONB,
                    NULL::JSONB,NULL::JSONB;
                RETURN;
            END IF;

            -- User check
            IF p_user_id IS NOT NULL AND p_user_id < 0 THEN
                RETURN QUERY SELECT 
                    'FAILURE','Invalid user ID provided',
                    NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
                    NULL,NULL,NULL,NULL,NULL,NULL,NULL::VARCHAR,NULL::BIGINT,NULL::JSONB,
                    NULL::JSONB,NULL::JSONB;
                RETURN;
            END IF;
                RETURN QUERY
                SELECT
                    'SUCCESS',
                    'Asset items retrieved successfully',
                    ai.id, a.id, a.name,
                    ai.model_number, ai.serial_number,
                    ai.thumbnail_image, ai.qr_code, ai.asset_tag,
                    CASE 
                        WHEN ai.assignee_type_id IS NOT NULL THEN
                            json_build_object(
                                'id', ai.assignee_type_id,
                                'name', at.name
                            )
                        ELSE NULL
                    END AS assignee_type,
                    ac.assets_type AS assets_type_id,
                    ast.name AS assets_type_name,
                    a.category AS category_id,
                    ac.name AS category_name,
                    a.sub_category AS sub_category_id,
                    assc.name AS sub_category_name,
                    ai.manufacturer,
                    ai.department AS department_id,
                    org.data AS department_data,
                    -- maintenance recommendations aggregate
                    (
                        SELECT jsonb_agg(
                            jsonb_build_object(
                                'id', amr.id,
                                'recommended_by_user_id', amr.recommended_by_user_id,
                                'user_name', u.user_name,
                                'message', amr.message,
                                'priority', amr.priority,
                                'recommendation_date', amr.recommendation_date,
                                'is_active', amr.is_active,
                                'created_at', amr.created_at,
                                'updated_at', amr.updated_at
                            ) ORDER BY amr.created_at DESC
                        )
                        FROM asset_maintenance_recommendations amr
                        LEFT JOIN users u ON amr.recommended_by_user_id = u.id
                        WHERE amr.asset_id = ai.id
                        AND amr.tenant_id = p_tenant_id
                        AND amr.is_active = TRUE
                    ) AS maintenance_recommendations,
                    -- asset recommendations aggregate
                    (
                        SELECT jsonb_agg(
                            jsonb_build_object(
                                'id', ar.id,
                                'message', ar.message,
                                'priority', ar.priority,
                                'recommendation_date', ar.recommendation_date,
                                'is_active', ar.is_active,
                                'created_at', ar.created_at,
                                'recommended_by_user_id', ar.recommended_by_user_id,
                                'recommendation_type', jsonb_build_object(
                                    'id', art.id,
                                    'code', art.code,
                                    'name', art.name
                                ),
                                'recommend_user_type', jsonb_build_object(
                                    'id', arut.id,
                                    'code', arut.code,
                                    'name', arut.name
                                )
                            ) ORDER BY ar.created_at DESC
                        )
                        FROM asset_recommendations ar
                        LEFT JOIN asset_recommendation_types art ON ar.recommendation_type_id = art.id
                        LEFT JOIN asset_recommend_user_types arut ON ar.recommend_user_type_id = arut.id
                        WHERE ar.asset_id = ai.id
                        AND ar.tenant_id = p_tenant_id
                        AND ar.is_active = TRUE
                    ) AS recommendations
                FROM asset_items ai
                INNER JOIN assets a ON ai.asset_id = a.id
                LEFT JOIN assignee_types at ON ai.assignee_type_id = at.id
                INNER JOIN asset_categories ac ON a.category = ac.id
                INNER JOIN assets_types ast ON ac.assets_type = ast.id
                INNER JOIN asset_sub_categories assc ON a.sub_category = assc.id
                INNER JOIN organization org ON ai.department = org.id
                WHERE (ai.id = p_asset_item_id OR p_asset_item_id IS NULL)
                AND ai.tenant_id = p_tenant_id
                AND ai.deleted_at IS NULL
                AND ai.isactive = TRUE
                AND a.deleted_at IS NULL
                AND a.isactive = TRUE;
            END IF;
        END;
        $$
        SQL);
    }

    public function down(): void
    {
        DB::unprepared('DROP FUNCTION IF EXISTS get_asset_details_with_recommendations(BIGINT, BIGINT, BIGINT, TEXT);');
    }
};
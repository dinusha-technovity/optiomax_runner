...
return new class extends Migration
{
    ...
    {
        DB::unprepared(<<<SQL
            CREATE OR REPLACE FUNCTION insert_or_update_asset_item_manufacturer_maintain_schedules(
            ...
            BEGIN
                ...
                IF p_asset_item IS NULL OR p_asset_item = 0 THEN
                    RETURN QUERY SELECT 'FAILURE', 'Asset item ID cannot be null or zero', NULL::JSONB, NULL::JSONB;
                    RETURN;
                END IF;
                ...
                IF p_schedule_id IS NULL OR p_schedule_id = 0 THEN
                    -- Insert new record
                    INSERT INTO asset_item_manufacturer_recommendation_maintain_schedules (
                        asset_item, 
                    ...
                    RETURNING id, to_jsonb(asset_item_manufacturer_recommendation_maintain_schedules.*) INTO p_inserted_or_updated_schedule_id, new_record;

                    RETURN QUERY SELECT 'SUCCESS', 'Manufacturer maintain schedules created successfully', NULL::JSONB, new_record;
                ELSE
                    -- Fetch old data before updating
                    SELECT to_jsonb(asset_item_manufacturer_recommendation_maintain_schedules.*) INTO old_record
                    FROM asset_item_manufacturer_recommendation_maintain_schedules
                    WHERE id = p_schedule_id;

                    -- Update existing record
                    UPDATE asset_item_manufacturer_recommendation_maintain_schedules
                    SET 
                    ...
                    SELECT to_jsonb(asset_item_manufacturer_recommendation_maintain_schedules.*) INTO new_record
                    FROM asset_item_manufacturer_recommendation_maintain_schedules
                    WHERE id = p_schedule_id;

                    RETURN QUERY SELECT 'SUCCESS', 'Manufacturer maintain schedules updated successfully', old_record, new_record;
                END IF;
            END;
            $$;
        SQL);
    ...
    {
        DB::unprepared('DROP FUNCTION IF EXISTS insert_or_update_asset_item_manufacturer_maintain_schedules');
    }
};

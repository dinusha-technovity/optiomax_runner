        ...
        DB::unprepared(<<<SQL
            ...
            BEGIN
                FOR r IN
                    ...
                    FROM pg_proc
                    WHERE proname = 'create_or_update_asset_items'
                LOOP
           ...
           CREATE OR REPLACE FUNCTION create_or_update_asset_items(
            ...
            AS $$
             DECLARE
                ...
                assigned_user_item JSON; -- Iterates over each assigned user in the assigned_users array
                is_schedule_available_val BOOLEAN; -- Stores the is_schedule_available value for the current item
            BEGIN
                ...
                LOOP
                    ...
                    ELSE
                        ...
                        BEGIN
                            ...
                            VALUES (
                                ...
                                (item->>'assignee_type_id')::BIGINT,
                                COALESCE((item->>'booking_availability')::BOOLEAN, true),
                                COALESCE((item->>'is_schedule_available')::BOOLEAN, true),
                            ...
                            -- Call supplier stats function once per asset item (after successful insert)
                            PERFORM change_supplier_asset_stats((item->>'supplier')::BIGINT, 'REGISTER', _tenant_id::BIGINT);
                            PERFORM calculate_supplier_rating((item->>'supplier')::BIGINT, 'ASSET_REGISTERED', '{}'::JSONB, _tenant_id::BIGINT);
                    ...
                    -- Process assigned users for the asset item (both insert and update)
                    -- Only process if is_schedule_available is false
                    IF _assigned_users IS NOT NULL AND is_schedule_available_val = false THEN
                        DECLARE
                            assigned_user_record JSON;
                            user_id_val BIGINT;
                        BEGIN
                            -- First, delete all existing assignments for this asset item
                            DELETE FROM asset_item_assigned_users
                            WHERE asset_item_id = current_item_id
                            AND tenant_id = _tenant_id;

                            -- Now insert the new assignments
                            FOR assigned_user_record IN SELECT * FROM json_array_elements(_assigned_users)
                            LOOP
                                user_id_val := (assigned_user_record->>'user_id')::BIGINT;
                                
                                -- If 'user_id' is null, try 'id'
                                IF user_id_val IS NULL THEN
                                    user_id_val := (assigned_user_record->>'id')::BIGINT;
                                END IF;

                                -- Check if this user assignment already exists
                                IF EXISTS (
                                    SELECT 1 FROM asset_item_assigned_users
                                    WHERE asset_item_id = current_item_id
                                    AND user_id = user_id_val
                                    AND tenant_id = _tenant_id
                                ) THEN
                                    -- Update existing assignment
                                    UPDATE asset_item_assigned_users
                                    SET

        ...
        DB::unprepared(<<<SQL
            ...
            BEGIN
                IF p_action_type = 'booking' THEN
                    -- Booking type - get only required documents for the specific asset
                    -- Query from asset_availability_required_documents_for_booking table
                    IF p_asset_id IS NULL THEN
                        RETURN QUERY
                        SELECT
                            'ERROR'::TEXT AS status,
                            'asset_id is required for booking action type'::TEXT AS message,
                            '[]'::jsonb AS data;
                        RETURN;
                    END IF;
                                ...
                                jsonb_build_object(
                                    ...
                                    'fields', COALESCE(
                                        (
                                            ...
                                            INNER JOIN asset_availability_required_documents_for_booking aardb
                                                ON aardb.document_category_field_id = dcf.id
                                                AND aardb.asset_items_id = p_asset_id
                                                AND aardb.deleted_at IS NULL
                    ...
                    AND EXISTS (
                        SELECT 1
                        FROM asset_availability_required_documents_for_booking aardb
                        INNER JOIN document_category_field dcf ON dcf.id = aardb.document_category_field_id
                        WHERE aardb.asset_items_id = p_asset_id
                        AND aardb.deleted_at IS NULL
                        AND dcf.document_category_id = dc.id
                    );

                ELSE
                    -- Normal type - categories with fields, with IDs
                    -- If asset_id is provided, get the required document field IDs for filtering

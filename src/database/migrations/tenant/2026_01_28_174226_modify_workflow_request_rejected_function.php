        ...
        DB::unprepared(<<<SQL
            ...
            BEGIN
                ...
                IF p_requisition_id IS NULL OR btrim(p_requisition_id) = '' THEN
                    RETURN QUERY SELECT 'ERROR', 'Requisition ID is required', NULL::JSONB;
                    RETURN;
                END IF;
                ...
                IF p_status = 'REJECT' THEN
                    CASE p_request_type_id
                        ...
                        IF p_additional_data IS NOT NULL AND p_additional_data->>'isUpdate' = 'true' THEN
                            ...
                            --     updated_at = p_current_time
                            -- WHERE supplier_reg_no = p_requisition_id;
                        ELSE
                            UPDATE public.suppliers
                            SET supplier_reg_status = 'REJECT',
                                updated_at = p_current_time
                            WHERE id = p_requisition_id::bigint;
                        ...
                        WHEN 3 THEN
                            ...
                            SET procurement_status = 'REJECT',
                                updated_at = p_current_time
                            WHERE request_id = p_requisition_id;
                        WHEN 4 THEN
                            UPDATE public.work_orders
                            SET status = 'REJECT',
                                updated_at = p_current_time
                            WHERE work_order_number = p_requisition_id;
                        WHEN 6 THEN
                        ...
                        WHEN 7 THEN
                            ...
                            SET status = 'Rejected',
                                updated_at = p_current_time
                            WHERE po_number = p_requisition_id;
                            ...
                            END IF;
                        WHEN 8 THEN
                            UPDATE public.internal_asset_requisitions

    ...
    public function up(): void
    {
        ...
        DO $$
            ...
            BEGIN
                FOR r IN
                    ...
                    FROM pg_proc
                    WHERE proname = 'reject_upgrade_decision'
                LOOP
                    EXECUTE format('DROP FUNCTION %s CASCADE;', r.func_signature);
                END LOOP;
            END$$;

        CREATE OR REPLACE FUNCTION reject_upgrade_decision(
            IN _tenant_id              BIGINT,
            IN _action_by              BIGINT,
            IN _decision_id            BIGINT,
            IN _asset_requisition_id   BIGINT,
            IN _rejection_reason       TEXT
        )
        RETURNS TABLE (
            status      TEXT,
            message     TEXT,
        ...
        BEGIN
            ...
            IF _rejection_reason IS NULL OR TRIM(_rejection_reason) = '' THEN
                RETURN QUERY SELECT 'FAILURE'::TEXT, 'Rejection reason is required'::TEXT, NULL::JSON;
                RETURN;
            END IF;

            -- ═══ Validate decision exists and is in PENDING status ═══
            SELECT ard.status
            ...
            IF v_decision_status <> 'PENDING' THEN
                RETURN QUERY SELECT 'FAILURE'::TEXT, ('Cannot reject: current status is ' || v_decision_status)::TEXT, NULL::JSON;
                RETURN;
            END IF;

            -- ═══ Get requisition number and asset owner info ═══
            ...
            SELECT u.name INTO v_action_by_name
            FROM users u
            WHERE u.id = _action_by;

            -- ═══ Get log type ID ═══
            SELECT id INTO v_log_type_id
            FROM asset_requisition_log_types
            WHERE code = 'ASSET_REQUISITION_REJECTED_BY_MAINTENANCE_LEADER'
              AND is_active = TRUE
            LIMIT 1;
            ...
            IF v_log_type_id IS NOT NULL THEN
                ...
                VALUES (
                    ...
                    v_log_type_id,
                    _action_by,
                    jsonb_build_object('rejection_reason', _rejection_reason, 'action_id', v_inserted_action_id),
                    NOW(),
                    NOW()
                );
            END IF;
            ...
            -- ═══ Step 5: Update upgrade_asset_requisitions status ═══
            ...
            v_result_json := json_build_object(
                ...
                'action_by_name', v_action_by_name
            );

            RETURN QUERY SELECT
                'SUCCESS'::TEXT,
                'Requisition rejected successfully'::TEXT,
                v_result_json;

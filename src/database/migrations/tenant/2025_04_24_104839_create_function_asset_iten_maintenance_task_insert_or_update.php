...
return new class extends Migration
{
    ...
    {
        DB::unprepared(<<<SQL
            CREATE OR REPLACE FUNCTION insert_or_update_asset_item_maintenance_task(
            ...
            BEGIN
                ...
                IF p_task_id IS NULL OR p_task_id = 0 THEN
                    -- Insert new record
                    INSERT INTO asset_item_maintenance_tasks (
                        asset_item, 
                    ...
                    RETURNING id, to_jsonb(asset_item_maintenance_tasks.*) INTO p_inserted_or_updated_task_id, new_record;
                    
                    RETURN QUERY SELECT 'SUCCESS', 'Record inserted successfully', NULL::JSONB, new_record;
                ELSE
                    -- Fetch old data before updating
                    SELECT to_jsonb(asset_item_maintenance_tasks.*) INTO old_record
                    FROM asset_item_maintenance_tasks
                    WHERE id = p_task_id;
                    
                    -- Update existing record
                    UPDATE asset_item_maintenance_tasks
                    SET 
                    ...
                    SELECT to_jsonb(asset_item_maintenance_tasks.*) INTO new_record
                    FROM asset_item_maintenance_tasks
                    WHERE id = p_task_id;
    ...
    {
        DB::unprepared('DROP FUNCTION IF EXISTS insert_or_update_asset_item_maintenance_task');
    }
};
        ...
        DB::unprepared(<<<SQL
            ...
            BEGIN
                FOR r IN
                    ...
                    FROM pg_proc
                    WHERE proname = 'manage_asset_item_assigned_users'
                LOOP
            ...
            CREATE OR REPLACE FUNCTION manage_asset_item_assigned_users(
            ...
            BEGIN
                ...
                ------------------------------------------------------------------
                -- 1️⃣ If no assigned users passed → unassign all
                ------------------------------------------------------------------
                IF _assigned_users IS NULL OR json_array_length(_assigned_users) = 0 THEN
                ...
                RETURN QUERY SELECT
                    'SUCCESS',
                    'Asset item assigned users updated successfully',
                    _asset_item_id;

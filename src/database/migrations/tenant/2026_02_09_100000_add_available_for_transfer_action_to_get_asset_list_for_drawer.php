...
{ 
    ...
    public function up(): void
    {
        ...
        BEGIN
            ...
            ELSIF p_action = 'available_for_transfer' THEN
                v_base_query := format($b$
                    FROM asset_items ai
                    INNER JOIN assets a ON ai.asset_id = a.id
                    INNER JOIN asset_categories ac ON a.category = ac.id
                    ...
                    WHERE ai.responsible_person = %s
                        ...
                        AND NOT EXISTS (
                            ...
                            WHERE rbatri.asset_item_id = ai.id
                            AND rbatri.asset_requester_approval_status = 'PENDING'
                            AND rbatri.deleted_at IS NULL
                            AND rbatri.isactive = TRUE
                        )
                        %s
                        %s
                $b$, p_user_id, p_asset_item_id, p_asset_item_id, p_asset_item_id, p_tenant_id, v_search_clause, v_category_clause);
                v_message := 'Assets available for transfer (excluding pending transfers) retrieved successfully';

    ...
    public function up(): void
    {
        ...
        DO $$
            ...
            BEGIN
                FOR r IN
                    ...
                    FROM pg_proc
                    WHERE proname = 'set_on_hold_upgrade_decision'
                LOOP
                    EXECUTE format('DROP FUNCTION %s CASCADE;', r.func_signature);
                END LOOP;
            END$$;

        CREATE OR REPLACE FUNCTION set_on_hold_upgrade_decision(
        ...
        BEGIN
            ...
            IF _reason IS NULL OR TRIM(_reason) = '' THEN
                RETURN QUERY SELECT 'FAILURE'::TEXT, 'Reason is required for on-hold action'::TEXT, NULL::JSON;
                RETURN;
            END IF;
            ...
            IF v_decision_status <> 'PENDING' THEN
                RETURN QUERY SELECT 'FAILURE'::TEXT, ('Cannot set on hold: current status is ' || v_decision_status)::TEXT, NULL::JSON;
                RETURN;
            END IF;
            ...
            WHERE code = 'ASSET_REQUISITION_SET_ON_HOLD'
              AND is_active = TRUE
            LIMIT 1;
            ...
            IF v_log_type_id IS NOT NULL THEN
                ...
                VALUES (
                    ...
                    v_log_type_id,
                    _action_by,
                    jsonb_build_object('reason', _reason, 'action_id', v_inserted_action_id),
                    NOW(),
                    NOW()
                );
            END IF;
            ...
            RETURN QUERY SELECT
                'SUCCESS'::TEXT,
                'Requisition placed on hold successfully'::TEXT,
                v_result_json;

...
return new class extends Migration
{
    public function up(): void
    {
        ...
        BEGIN 
            FOR r IN
                ...
                FROM pg_proc
                WHERE proname = 'get_employee_asset_scheduling_occurrences'
            LOOP
                EXECUTE format('DROP FUNCTION %s CASCADE;', r.func_signature);
            END LOOP;
        END$$;

        CREATE OR REPLACE FUNCTION get_employee_asset_scheduling_occurrences(
        ...
        BEGIN
            ...
            LEFT JOIN asset_schedule_related_employees asre ON s.id = asre.asset_schedule_id
            WHERE (p_schedule_id IS NULL OR s.id = p_schedule_id)
            AND s.tenant_id = p_tenant_id
            AND (p_asset_id IS NULL OR s.asset_id = p_asset_id)
            AND (p_employee_id IS NULL OR asre.employee_id = p_employee_id)
            ...
            SELECT DISTINCT ON (occ.id)
                ...
                (
                    ...
                    JOIN users u ON asre2.employee_id = u.id
    ...
    {
        DB::unprepared("DROP FUNCTION IF EXISTS get_employee_asset_scheduling_occurrences(TEXT, BIGINT, TEXT, BIGINT, BIGINT, BIGINT, TIMESTAMPTZ, TIMESTAMPTZ);");
    }
};

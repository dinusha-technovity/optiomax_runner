...
return new class extends Migration
{
    ...
    {  
        ...
        BEGIN
            ...
            FOR r IN
                ...
                FROM pg_proc
                WHERE proname = 'get_asset_categories_list_paginated'
            LOOP
                EXECUTE format('DROP FUNCTION %s CASCADE;', r.func_signature);
            END LOOP;
        END$$;

        CREATE OR REPLACE FUNCTION get_asset_categories_list_paginated(
            p_tenant_id BIGINT,
        ...
        DECLARE
            ...
            v_offset_curr INT;
            v_offset_next INT;
            v_order_clause TEXT := 'ORDER BY ac.id DESC';
            inner_sql TEXT;
        BEGIN
            -- Sorting logic
            CASE LOWER(TRIM(p_sort_by))
                WHEN 'newest' THEN v_order_clause := 'ORDER BY ac.id DESC';
                WHEN 'oldest' THEN v_order_clause := 'ORDER BY ac.id ASC';
                WHEN 'az' THEN v_order_clause := 'ORDER BY ac.name ASC NULLS LAST';
                WHEN 'za' THEN v_order_clause := 'ORDER BY ac.name DESC NULLS LAST';
                ELSE v_order_clause := 'ORDER BY ac.id DESC';
            END CASE;
            ...
            AND (p_asset_category_id IS NULL OR ac.id = p_asset_category_id)
            AND (p_search IS NULL OR ac.name ILIKE '%' || p_search || '%' OR ac.description ILIKE '%' || p_search || '%');
            ...
            inner_sql := format($SQL$
                ...
                FROM asset_categories ac
                LEFT JOIN assets_types at ON ac.assets_type = at.id
                WHERE ac.tenant_id = %L
                AND ac.deleted_at IS NULL
                AND ac.isactive = TRUE
                AND (%L IS NULL OR ac.id = %L)
                AND (%L IS NULL OR ac.name ILIKE '%%' || %L || '%%' OR ac.description ILIKE '%%' || %L || '%%')
                %s
                LIMIT %s OFFSET %s
            $SQL$,
                p_tenant_id, p_asset_category_id, p_asset_category_id,
            ...
            RETURN json_build_object(
                'status', 'SUCCESS',
                'message', CASE WHEN p_asset_category_id IS NULL THEN 'Asset categories fetched successfully' ELSE 'Asset category details fetched successfully' END,
                'meta', json_build_object(
    ...
    public function down(): void
    {
       DB::unprepared(<<<'SQL'
        ...
        -- Recreate the original function without asset_type
        CREATE OR REPLACE FUNCTION get_asset_categories_list_paginated(
            p_tenant_id BIGINT,
            p_asset_category_id INT DEFAULT NULL,
            p_page_no INT DEFAULT 1,

        ...
        BEGIN
            ...
            IF p_tenant_id IS NULL OR p_tenant_id <= 0 THEN
                RETURN QUERY SELECT 
                    ...
                    NULL::JSONB,NULL::TIMESTAMP,NULL::TEXT;
                RETURN;
            END IF;
                    ...
                    (
                       SELECT jsonb_agg(
                            jsonb_build_object(
                                ...
                                'recommendation_type_id', amr.recommendation_type_id,
                                'recommendation_type_code', art.code,
                                'recommend_user_type_id', amr.recommend_user_type_id,
                                'recommend_user_type_code', arut.code,
                                'created_at', amr.created_at,
                                'updated_at', amr.updated_at
                            )
                            ORDER BY amr.created_at DESC
                        )
                        FROM asset_recommendations amr
                        INNER JOIN users u 
                            ON amr.recommended_by_user_id = u.id
                        INNER JOIN asset_recommendation_types art 
                            ON amr.recommendation_type_id = art.id
                        INNER JOIN asset_recommend_user_types arut 
                            ON amr.recommend_user_type_id = arut.id
                        WHERE amr.asset_id = ai.id
                        AND amr.tenant_id = p_tenant_id
                        AND amr.is_active = TRUE
                    ) AS maintenance_recommendations,
                    ai.created_at AS registered_date,

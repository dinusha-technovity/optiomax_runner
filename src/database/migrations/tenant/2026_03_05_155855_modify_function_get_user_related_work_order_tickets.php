        ...
        DB::unprepared(<<<SQL
            ...
            BEGIN 
                FOR r IN
                    ...
                    FROM pg_proc
                    WHERE proname = 'get_user_related_work_order_tickets'
                LOOP
            ...
            BEGIN
                ...
                SELECT
                    ...
                    CASE 
                        ...
                        WHEN wot.type = 'asset_upgrade_request' THEN (
                            SELECT to_jsonb(aur)
                            FROM (
                                SELECT 
                                    uar.id,
                                    uar.tenant_id,
                                    uar.asset_requisition_id,
                                    ar.requisition_id::TEXT AS asset_requisition_number,
                                    ar.requisition_status::TEXT AS asset_requisition_status,
                                    uar.asset_id,
                                    ai.asset_tag::TEXT AS asset_tag,
                                    ai.serial_number::TEXT AS asset_serial_number,
                                    ai.model_number::TEXT AS asset_model_number,
                                    ...
                                    uar.priority,
                                    wopl.name::TEXT AS priority_level_name,
                                    uar.expected_date,
                                    uar.error_logs_performance_doc,
                                    uar.screenshots,
                                    uar.other_docs,
                                    uar.notified_maintenance_leaders,
                                   
                                    uar.status,
                                    uar.is_recommend_for_transition,
                                    uar.transition_log_id,
                                    art.from_type::TEXT AS transition_from_type,
                                    art.to_type::TEXT AS transition_to_type,

                                    uar.is_active,
                                    uar.created_by,
                                    uc.name::TEXT AS created_by_name,
                                    uc.email::TEXT AS created_by_email,
                                    uar.created_at,
                                    uar.updated_at,
                                    uar.deleted_at
                                FROM upgrade_asset_requisitions uar
                                LEFT JOIN asset_requisitions ar ON uar.asset_requisition_id = ar.id
                                LEFT JOIN asset_items ai ON uar.asset_id = ai.id
                                LEFT JOIN assets a ON ai.asset_id = a.id
                                LEFT JOIN work_order_priority_levels wopl ON uar.priority = wopl.id
                                LEFT JOIN asset_requisition_type_transition art ON uar.transition_log_id = art.id
                                LEFT JOIN users uc ON uar.created_by = uc.id
                                WHERE uar.asset_id = wot.asset_id

...
return new class extends Migration
{
    ...
    {
        DB::unprepared(<<<SQL
            CREATE OR REPLACE FUNCTION set_asset_availability_schedule_publish_status(
            ...
            BEGIN
                ...
                IF NOT FOUND THEN
                    RETURN QUERY SELECT 
                        ...
                        'Schedule not found.'::TEXT AS message;
                    RETURN;
                END IF;
                ...
                IF p_action = 'PUBLISH' THEN
                    IF v_current_status NOT IN ('DRAFT', 'UNPUBLISHED') THEN
                        RETURN QUERY SELECT 
                            'FAILURE'::TEXT AS status,
                            'Only schedules with publish_status = DRAFT or UNPUBLISH can be published'::TEXT AS message;
                        RETURN;
                    END IF;
                ...
                ELSIF p_action = 'UNPUBLISHED' THEN
                    IF v_current_status <> 'PUBLISH' THEN
                        RETURN QUERY SELECT 
                            'FAILURE'::TEXT AS status,
                            'Only schedules with publish_status = PUBLISH can be unpublished'::TEXT AS message;
                        RETURN;
                    END IF;
                    UPDATE asset_availability_schedules
                ...
                ELSE
                    RETURN QUERY SELECT 
                        'FAILURE'::TEXT AS status,
                        'Invalid action. Use PUBLISH or UNPUBLISH.'::TEXT AS message;
                    RETURN;
                END IF;
                ...
                IF rows_updated > 0 THEN
                    ...
                    IF p_user_id IS NOT NULL AND p_user_name IS NOT NULL THEN
                        BEGIN
                            PERFORM log_activity(
                                CASE WHEN p_action = 'PUBLISH' THEN 'asset_availability_schedule.published'
                                    ELSE 'asset_availability_schedule.unpublished' END,
                                'Schedule ' || lower(p_action) || 'ed by ' || p_user_name || ': ' || p_asset_availability_schedule_id,
                    ...
                    RETURN QUERY SELECT 
                        ...
                        'Asset Availability Schedule ' || lower(p_action) || 'ed successfully'::TEXT AS message;
                ELSE
                    RETURN QUERY SELECT 
                        'FAILURE'::TEXT AS status,
                        'No rows updated. Schedule not found or already in desired state.'::TEXT AS message;
                END IF;
            END;
            $$;
        SQL);
    ...
    {
        DB::unprepared('DROP FUNCTION IF EXISTS set_asset_availability_schedule_publish_status(BIGINT, TEXT, BIGINT, BIGINT, TIMESTAMPTZ, TIMESTAMPTZ)');
    }
};
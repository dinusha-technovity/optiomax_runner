...
return new class extends Migration
{
    /**
     * Run the migrations.
     * Fixes the replace section of get_user_asset_requisitions:
     * - Removed dropped columns: replace_requisition_number, other_docs, work_order_id,
     *   upgrade_action_id, is_disposal_recommended, disposal_recommended_type
     * - Added missing columns: specifications, notified_maintenance_leaders
     * - Removed suppliers subquery (no longer relevant for replace requisitions)
     */
    public function up(): void
    {
        ...
        BEGIN
            ...
            -- ══════════════════════════════════════════════════════════════════
            -- REPLACE REQUISITIONS (CORRECTED)
            -- ══════════════════════════════════════════════════════════════════
            ELSIF v_req_type = 'replace' THEN
                ...
                EXECUTE format($SQL$
                    ...
                    FROM (
                        SELECT
                            ...
                            rar.screenshots,
                            rar.specifications,
                            rar.notified_maintenance_leaders,
                            ...
                            -- Action details (for REJECTED, ON_HOLD, UPGRADE_SUGGESTED)
                            CASE
                                WHEN rar.status IN ('REJECTED', 'ON_HOLD', 'UPGRADE_SUGGESTED') THEN
                                    COALESCE(
                                        (
                                            SELECT json_build_object(
                                                'id', act.id,

...
return new class extends Migration
{
    ...
    public function up(): void
    {
        DB::unprepared(<<<SQL
            ...
            DROP FUNCTION IF EXISTS bulk_insert_asset_sub_categories_with_relationships(BIGINT, BIGINT, BIGINT, TIMESTAMP WITH TIME ZONE, JSON, INTEGER);

            -- Function to handle asset sub-category bulk import with automatic parent category creation
            ...
            DECLARE
                ...
                categories_created_count BIGINT := 0;
            ...
            BEGIN
                ...
                LOOP
                    ...
                    BEGIN
                        ...
                        -- If asset category not found, create it automatically
                        IF asset_category_id_val IS NULL THEN
                            BEGIN
                                -- Create the parent asset category
                                ...
                                VALUES (
                                    ...
                                    format('Auto-created category for sub-category: %s', current_name),
                                    NULL,
                                    true,
                                    _tenant_id,
                                    _created_by_user_id,
                                ...
                                INSERT INTO temp_existing_categories (category_name_lower, category_id)
                                VALUES (LOWER(current_category_name), asset_category_id_val)
                                ON CONFLICT (category_name_lower) DO NOTHING;
                                
                                categories_created_count := categories_created_count + 1;
                                
                            EXCEPTION WHEN unique_violation THEN
                                -- Handle race condition - category was created by another process
                                SELECT id INTO asset_category_id_val
                                FROM asset_categories
                                WHERE LOWER(name) = LOWER(current_category_name)
                                AND tenant_id = _tenant_id
                                AND isactive = true
                                LIMIT 1;
                                
                                IF asset_category_id_val IS NOT NULL THEN
                                    -- Update temp table cache
                                    INSERT INTO temp_existing_categories (category_name_lower, category_id)
                                    VALUES (LOWER(current_category_name), asset_category_id_val)
                                    ON CONFLICT (category_name_lower) DO UPDATE SET category_id = EXCLUDED.category_id;
                                ELSE
                                    -- Still couldn't find it, add error
                                    error_count := error_count + 1;
                                    error_details_array := array_append(error_details_array, 
                                        json_build_object(
                                            'row', processed_count,
                                            'name', current_name,
                                            'error', format('Failed to create or find asset category: %s', current_category_name)
                                        )
                                    );
                                    CONTINUE;
                                END IF;
                ...
                RETURN QUERY SELECT
                    ...
                    format('Processed %s asset sub-categories: %s inserted, %s updated, %s errors, %s categories auto-created', 
                        processed_count, inserted_count, updated_count, error_count, categories_created_count)::TEXT AS message,
                    ...
                    json_build_object(
                        ...
                        'categories_auto_created', categories_created_count,
    ...
    {
        // Restore the original function that doesn't auto-create categories
        DB::unprepared(<<<SQL
            DROP FUNCTION IF EXISTS bulk_insert_asset_sub_categories_with_relationships(BIGINT, BIGINT, BIGINT, TIMESTAMP WITH TIME ZONE, JSON, INTEGER);
            
            -- Recreate the original version that errors on missing categories
            -- (The original migration would need to be rerun, or copy the original function here)
        SQL);
    }
};

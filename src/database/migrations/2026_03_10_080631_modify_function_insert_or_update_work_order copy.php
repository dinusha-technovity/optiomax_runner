    ...
    public function up(): void
    {
        DB::unprepared(<<<SQL
            ...
            CREATE OR REPLACE FUNCTION insert_or_update_work_order(
                ...
                p_asset_requisition_id BIGINT DEFAULT NULL
            )
            RETURNS TABLE (
            ...
            BEGIN
                ...
                IF p_id = 0 THEN
                    INSERT INTO work_orders (
                        ...
                        tenant_id, user_id, work_order_ticket_id, asset_requisition_id,
                        ...
                        p_tenant_id, p_user_id, p_work_order_ticket_id, p_asset_requisition_id,
                        NOW(), NOW()
                        ) RETURNING id INTO v_work_order_id;
                        IF p_work_order_ticket_id IS NOT NULL THEN
                        UPDATE work_order_tickets
                        SET is_get_action = TRUE,
                            updated_at = NOW()
                        WHERE id = p_work_order_ticket_id;
                    END IF;
                    ELSE
                    UPDATE work_orders SET
                        ...
                        -- only overwrite actual_work_order_start/end when new values are provided
                        actual_work_order_start = COALESCE(p_actual_work_order_start, actual_work_order_start),
                        actual_work_order_end = COALESCE(p_actual_work_order_end, actual_work_order_end),

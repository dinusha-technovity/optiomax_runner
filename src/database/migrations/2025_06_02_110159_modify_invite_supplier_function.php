    ...
    public function up(): void
    {
        DB::unprepared(<<<SQL
            DROP FUNCTION IF EXISTS invite_supplier(
                VARCHAR, VARCHAR, BIGINT, TIMESTAMP, TIMESTAMP, BIGINT, VARCHAR
            );
            ...
            BEGIN
                ...
                -- Check if supplier exists with same email
                SELECT s.id, s.supplier_reg_no INTO generated_supplier_id, new_supplier_reg_no
                FROM suppliers s
                WHERE s.tenant_id = p_tenant_id AND s.email = p_email
                LIMIT 1;

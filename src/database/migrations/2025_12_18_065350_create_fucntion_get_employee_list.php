...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DECLARE
            ...
            v_offset_curr INT := 0;
            v_offset_next INT := 0;
            v_order_clause TEXT := 'ORDER BY e.id DESC';
            v_message TEXT := '';
            v_base_query TEXT := '';
        ...
        BEGIN
            ...
            IF p_search IS NOT NULL AND LENGTH(TRIM(p_search)) > 0 THEN
                v_search_clause := format(
                    'AND (
                        e.employee_name ILIKE %L 
                        OR e.email ILIKE %L 
                        OR e.employee_number ILIKE %L
                        OR e.phone_number ILIKE %L
                        OR CONCAT(COALESCE(c.phone_code, ''''), '' '', COALESCE(e.phone_number, '''')) ILIKE %L
                        OR CONCAT(''+'', COALESCE(c.phone_code, ''''), '' '', COALESCE(e.phone_number, '''')) ILIKE %L
            ...
            v_base_query := format(
                'FROM employees e
                LEFT JOIN countries c ON e.contact_no_code = c.id
                LEFT JOIN designations d ON e.designation_id = d.id
                LEFT JOIN organization o ON e.department = o.id
            ...
            EXECUTE format(
                'SELECT COALESCE(json_agg(t), ''[]''::JSON) FROM (
                    SELECT 
                        ...
                        e.phone_number::TEXT,
                        e.contact_no_code,
                        c.phone_code::TEXT AS country_code,
                        e.address::TEXT,

...
return new class extends Migration
{
        ...
        BEGIN
            ...
            IF p_action = 'responsible' THEN
                v_base_query := format(\$BASE$
                    ...
                    WHERE ai.responsible_person = %s
                        AND (ai.id = %s OR %s IS NULL OR %s <= 0)
                        AND ai.tenant_id = %s
                        ...
                        %s
                        %s
                \$BASE$, p_user_id, p_asset_item_id, p_asset_item_id, p_asset_item_id, p_tenant_id, v_search_clause, v_category_clause);
            ...
            ELSIF p_action = 'user_related' THEN
                v_base_query := format(\$BASE$
                    ...
                    WHERE mtm.user_id = %s
                        ...
                        %s
                        %s
                \$BASE$, p_user_id, p_tenant_id, v_search_clause, v_category_clause);
            ...
            ELSIF p_action = 'availability' THEN
                v_base_query := format(\$BASE$
                    ...
                    INNER JOIN assets_types ast ON ac.assets_type = ast.id
                    INNER JOIN asset_sub_categories assc ON a.sub_category = assc.id
                    WHERE (ai.id = %s OR %s IS NULL OR %s <= 0)
                        AND ai.tenant_id = %s
                        AND ai.deleted_at IS NULL
                        AND ai.isactive = TRUE
                        AND a.deleted_at IS NULL
                        ...
                        %s
                        %s
                \$BASE$, p_asset_item_id, p_asset_item_id, p_asset_item_id, p_tenant_id, p_tenant_id, v_search_clause, v_category_clause);
            ...
            ELSE
                ...
                v_base_query := format(\$BASE$
                    ...
                    WHERE (ai.id = %s OR %s IS NULL)
                        ...
                        %s
                        %s
                \$BASE$, p_asset_item_id, p_asset_item_id, p_tenant_id, v_search_clause, v_category_clause);
    ...
    {
        DB::unprepared('DROP FUNCTION IF EXISTS get_asset_details_for_master_entry(BIGINT, BIGINT, BIGINT, TEXT, INT, INT, TEXT, TEXT, TEXT, BIGINT);');
    }
};
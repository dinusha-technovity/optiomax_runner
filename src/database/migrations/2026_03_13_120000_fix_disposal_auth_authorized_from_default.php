...
return new class extends Migration
{
     ...
     * Run the migrations.
     * 
     * Fix: Remove required validation for authorized_from and add default to CURRENT_DATE
     * This allows creating disposal authorizations without explicitly providing authorized_from
     */
    public function up(): void
    {
        ...
        DB::unprepared(<<<'SQL'
            ...
            DECLARE
                ...
                v_authorized_from_date DATE;
            BEGIN
                -- Default authorized_from to CURRENT_DATE if NULL
                v_authorized_from_date := COALESCE(p_authorized_from, CURRENT_DATE);
                ...
                IF p_disposal_auth_id IS NULL THEN
                    ...
                    -- Validate date range if authorized_until provided
                    IF p_authorized_until IS NOT NULL THEN
                        IF p_authorized_until < v_authorized_from_date THEN
                            RETURN jsonb_build_object(
                                'status', 'ERROR',
                ...
                ELSE
                    ...
                    -- INSERT new authorization (use v_authorized_from_date which defaults to CURRENT_DATE)
                    INSERT INTO disposal_authorization_users (
                        ...
                        v_authorized_from_date, -- Use variable with default
                        p_authorized_until,
                    ...
                    BEGIN
                        v_log_id := log_activity(
                            ...
                            jsonb_build_object(
                                ...
                                'authorized_from', v_authorized_from_date,
                                'action', 'assign',
                                'new_data', v_new_data,
    ...
    public function down(): void
    {
        ...
        // Note: The down migration would need to restore the old function
        // For simplicity, just drop it - the previous migration can be re-run if needed
    }
};
